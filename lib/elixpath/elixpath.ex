defmodule Elixpath do
  ...
  * `atom_keys_preferred` (a) - passes `prefer_keys: :atom` option to `Elixpath.Parser.parse/2`.
  ...
  defp do_query(data, %Elixpath{path: [Elixpath.PathComponent.child(key)]}, _gots, opts) do
    IO.inspect(key)
  ...
  defp do_query(
         data,
         %Elixpath{path: [Elixpath.PathComponent.child(key) | rest]} = path,
         _gots,
         opts
       ) do
    IO.inspect(key)
  ...
  @spec update(data :: term, t | String.t(), any) ::
          term
  def update(data, path_or_str, opts \\ "")

  def update(data, str, opts) when is_binary(str) do
    with {:ok, compiled_path} <- Elixpath.Parser.parse(str, []) do
      update(data, compiled_path, opts)
    end
  end

  def update(data, %Elixpath{} = path, opts) do
    do_update(data, path, opts)
  end

  @spec do_update(term, t, any) :: term
  defp do_update(data, %Elixpath{path: []}, _opts), do: data

  defp do_update(data, %Elixpath{path: [PathComponent.child(key)]}, update_val) do
    Elixpath.Update.update(data, key, update_val)
  end

  defp do_update(data, %Elixpath{path: [PathComponent.child(key) | rest]}, update_val)
       when is_map(data) do
    case Elixpath.Access.query(data, key, []) do
      {:ok, [sub_data]} ->
        new_data = do_update(sub_data, %Elixpath{path: rest}, update_val)
        updated_data = Map.put(data, key, new_data)
        updated_data
  ...
  defp do_update(
         data,
         %Elixpath{path: [PathComponent.child(Tag.wildcard()) | rest]},
         update_val
       )
    ...
    new_data =
      data
      |> Enum.map(fn v ->
        new_val = do_update(v, %Elixpath{path: rest}, update_val)
        new_val
      end)
  ...
  defp do_update(data, %Elixpath{path: [PathComponent.child(key) | rest]}, update_val)
       when is_list(data) do
    target_data = Enum.at(data, key)
    updated_target_data = do_update(target_data, %Elixpath{path: rest}, update_val)
    new_data = List.update_at(data, key, fn _v -> updated_target_data end)
    new_data
  end
